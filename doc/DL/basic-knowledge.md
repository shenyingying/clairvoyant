...
5. 色彩抖动
## 数据预处理
1. 中心归一化：计算训练集图像像素均值，之后在处理训练集、验证集、和测试集图像时需要分别减去该均值;

    
     注意：在实际操作中应先分好训练集、验证集、和测试集、而该均值仅针对划分后的训练集计算，不可直接在未划分的所有
     图像上进行计算，因为这违背了机器学习的基本原理“模型训练过程能且仅能从训练数据中获取信息”;
## 网络初始化
网络参数初始化方案：
...
|全零初始化|  |  |  无法训练 |
|随机初始化| 随机设定为接近0的高斯分布 |  |  |
|Xavier参数初始化| 均值为0,方差为sqrt（n）|实验对比发现该初始化方法有更快的收敛速率|由于该方法为考虑非线性映射的输出 所以它并不能完全符合实际情况|
|He参数初始化|均值为0,方差为sqrt(n/2) |比Xavier有更快的收敛速度 | |
|其他初始化方法|利用预训练模型参数作为初始化参数 | |  |
|数据敏感的参数初始化方式|根据自身任务数据集量身定制的参数初始化方法 | | |

## 激活函数

|名称 | 公式 |优点   | 缺点   |
|:---|:----|:-----|:-------|
|sigmoid|![](activate/sigmoid.png)|模仿了激活的特性|1.均值不为0; 2.容易造成梯度爆炸或梯度弥散|
|tanh|![](activate/tanh.png)|解决sigmoid均值不为0|会出现“梯度饱和”现象|
|ReLU|![](activate/relu.png)|1.完全消除了“梯度饱和”效应; 2.降低了计算复杂度; 3.有助于随机梯度下降方法的收敛（增加6倍左右）|x<0,梯度为0,一旦为负，将无法影响网络|
|LeakyReLU|![](activate/leakyRelu.png)|缓解relu“死区”现象|a为超参数，合适的值较难设定且较为敏感，在使用过程中不太稳定|
|参数化ReLU|作为网络中学习的变量融入模型的整体训练过程; 用的是传统的误差反向传播和随机梯度下降;|参数relu>普通RELU; 独享参数relu>共享参数relu; 深层网络所需的非线性能力较强|增加了模型过拟合的风险|
...
|:-----|:----|:-----|:-------|
|分类任务的目标函数： | | | |
|简单的衡量模型预测值和样本真实标记之间的误差：  |
|交叉熵损失函数| softmax损失函数:通过网络输出h转换为概率| ![softmax](objectFunc/softmax.png)|目前卷积神经网络中常用的分类函数|
|合页损失函数|对错误越大的样本施加越严重的惩罚 |![softmax](objectFunc/high.png)|一般情况下分类任务交叉熵损失函数分类效果略优于合页损失函数|
|坡道损失函数|在误差较大区域进行了“截断”|![softmax](objectFunc/ramp.png)|![softmax](objectFunc/ramp-high.png)|
| 考虑到特征表示的判别性：| |  | |
|大间隔交叉熵损失函数|为使特征更具分辨能力则引入参数m“拉大”二者差距 |![softmax](objectFunc/ls.png) |其不仅要求分类正确且要求分开的类别保持较大间隔 使得训练目标更难训练 可以防止过拟合 | 
|中心损失函数|在考虑类间距离的同时还将注意里放在减小类内差异 | ![softmax](objectFunc/center.png)| 参数越大类内差异占整个目标函数较大比重; 中心损失函数要优于交叉熵损失函数 |
|回归任务的目标函数：衡量模型预测值和真实值之间的靠近程度 | | | |
|L1函数： |  | ![softmax](objectFunc/l1.png)  | |
|L2函数： |  | ![softmax](objectFunc/l2.png)| 某些情况下l2 优于l1损失函数; 收敛速度方面l2较快 |
|tukey biweight函数： | 是一类非凸损失函数 | ![softmax](objectFunc/tb.png)| 克服回归任务中离群点或样本噪声对整体回归模型的干扰和影响  |
|标记分布|  |  |  |